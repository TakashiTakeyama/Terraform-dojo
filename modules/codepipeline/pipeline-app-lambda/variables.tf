...
variable "trigger" {
  description = "V2 トリガー（ブランチ・パス・タグ）。null で従来の DetectChanges に委ねる。"
  type = object({
...
variable "source_output_artifact_format" {
  description = "Lambda 向けは通常 CODE_ZIP。"
  type        = string
  default     = "CODE_ZIP"
...
variable "build_buildspec_path" {
  description = "buildspec_inline が空のとき、ソースルートからの相対パス。"
  type        = string
...
variable "build_compute_type" {
  description = "Build 段のコンピュートタイプ。"
  type        = string
  default     = "BUILD_GENERAL1_SMALL"
}

variable "build_privileged_mode" {
  description = "Lambda 向けビルドでは通常 false。"
  type        = bool
  default     = false
}
...
variable "build_stage_iam_policy_json" {
  description = "ビルド段のみに付与する追加 IAM（例: S3 から依存取得）。"
  type        = string
  default     = null
}

variable "build_namespace" {
  description = "後続 Deploy で参照する exported-variables 用 namespace（任意）。"
  type        = string
  default     = null
}
...
variable "deploy_stage_iam_policy_json" {
  description = "デプロイ段に付与する IAM（lambda:UpdateFunctionCode 等）。本番は最小権限の JSON を推奨。"
  type        = string
  default     = null
}
